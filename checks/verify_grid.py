...
for lat in latitudes:
    ...
    print(f"  Cell dimensions: {ns_dist:.2f} km (N-S) × {ew_dist:.2f} km (E-W) = {cell_area:.2f} km²")
    print(f"  Deviation from target: N-S {abs(ns_dist-grid_size_km)/grid_size_km*100:.1f}%, E-W {abs(ew_dist-grid_size_km)/grid_size_km*100:.1f}%")
    print()
...
print(f"  Longitude cells per latitude band: varies from {min(total_lon_cells_by_lat)} to {max(total_lon_cells_by_lat)}")
print(f"  Average longitude cells per latitude band: {avg_lon_cells:.1f}")
print(f"  Total grid cells: {total_cells} (compared to {total_lat_cells * 214} with fixed longitude spacing)")

# Calculate the geographic coverage and theoretical ideal grid size
print("\nGeographic Coverage:")
ns_distance = (max_lat - min_lat) * KM_PER_LAT_DEGREE
ew_distance_avg = (max_lon - min_lon) * KM_PER_LAT_DEGREE * np.cos(np.radians((min_lat + max_lat) / 2))
...
print(f"  Theoretical perfect grid cell count: {int(area / (grid_size_km * grid_size_km))}") 