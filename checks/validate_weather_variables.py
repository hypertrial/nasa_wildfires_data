...
def safe_zscore(values):
    ...
    std = np.std(values)
    if std < 1e-6:
        print("  NOTE: Very little variation in data (std < 1e-6), skipping outlier detection")
...
def validate_weather_variables(file_path, data_type='current', output_dir='output', generate_plots=True):
    """
    ...
    for var in weather_vars:
        ...
        if out_of_range_count > 0:
            print(f"  WARNING: {out_of_range_count} values ({out_of_range_percent:.2f}%) are outside the expected range")
            results['out_of_range'].append((var, out_of_range_count, out_of_range_percent))
            results['overall_valid'] = False
        else:
            print(f"  ✓ All values are within expected range")
        ...
        if value_range < 1e-6:
            print(f"  NOTE: All values are identical or nearly identical ({actual_min})")
            outlier_count = 0
            outlier_percent = 0
        else:
        ...
        if outlier_count > 0:
            print(f"  Outliers: {outlier_count} values ({outlier_percent:.2f}%) are statistical outliers")
        ...
        print(f"  Statistics: mean={mean_val:.2f}, median={median_val:.2f}, std={std_val:.2f} {units}")
        
        # 4. Generate visualization plots if requested
        if generate_plots:
            # Skip detailed plots if all values are identical
            if value_range < 1e-6:
                print(f"  NOTE: Skipping plots because all values are identical")
                continue
            ...
            ax1.axvline(x=min_val, color='r', linestyle='--', alpha=0.7, label=f'Min expected: {min_val}')
            ax1.axvline(x=max_val, color='r', linestyle='--', alpha=0.7, label=f'Max expected: {max_val}')
            ...
            if std_val > 1e-6:
                ...
                ax1.axvline(x=lower_threshold, color='g', linestyle=':', alpha=0.7, label=f'Outlier threshold (lower)')
                ax1.axvline(x=upper_threshold, color='g', linestyle=':', alpha=0.7, label=f'Outlier threshold (upper)')
            ax1.legend()
            ...
            if data_type == 'current' and len(df) < 10000:  # Only for reasonably sized datasets
                # Group by timestamp and calculate statistics
                time_stats = df.groupby(df['timestamp'].dt.date)[var].agg(['mean', 'min', 'max'])
                
                if len(time_stats) > 1:  # Only create time series if we have multiple dates
                    plt.figure(figsize=(14, 6))
                    plt.plot(time_stats.index, time_stats['mean'], marker='o', label='Mean')
                    plt.fill_between(time_stats.index, time_stats['min'], time_stats['max'], alpha=0.2, label='Min-Max Range')
                    plt.axhline(y=min_val, color='r', linestyle='--', alpha=0.7, label=f'Min expected: {min_val}')
                    plt.axhline(y=max_val, color='r', linestyle='--', alpha=0.7, label=f'Max expected: {max_val}')
                    ...
                    time_plot_file = os.path.join(output_dir, f'{data_type}_{var}_time_series.png')
        ...
        try:
            for var in weather_vars:
                ...
                if len(time_series) > 2:
                    ...
                    if std_change > 1e-6:
                        large_jumps = np.abs(changes) > (4 * std_change)  # More than 4 standard deviations
    ...
    if results['overall_valid']:
        ...
        print("The data may still be usable but should be reviewed carefully before analysis.")
        return False

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python validate_weather_variables.py <weather_data_file> <data_type> [output_dir] [generate_plots]")
    ...
    generate_plots = True if len(sys.argv) <= 4 else sys.argv[4].lower() in ['true', 't', 'yes', 'y', '1']
    ...
    if not success:
        print("Weather variable validation detected issues. Review the data before proceeding.")
        sys.exit(1)
