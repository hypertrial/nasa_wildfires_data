...
def verify_firms_data(file_path, min_lat=30.0, max_lat=70.0, min_lon=-130.0, max_lon=-100.0):
    """
    ...
    if df['longitude'].min() < min_lon or df['longitude'].max() > max_lon:
        print(f"WARNING: Longitude values outside expected range [{min_lon}, {max_lon}]")
    ...
    if 'confidence' in df.columns:
        if df['confidence'].min() < 0 or df['confidence'].max() > 100:
            ...
            print(f"  - Found range: [{df['confidence'].min()}, {df['confidence'].max()}]")
    ...
    if 'acq_date' in df.columns:
        try:
            ...
            print(f"Temporal coverage: {date_range[0].date()} to {date_range[1].date()} ({days_covered} days)")
            ...
            if missing_dates:
                print(f"WARNING: Found {len(missing_dates)} days with missing fire data:")
                for date in sorted(list(missing_dates))[:5]:  # Show first 5 missing dates
                    print(f"  - {date}")
    ...
    duplicates = df.duplicated(subset=['latitude', 'longitude', 'acq_date', 'acq_time']).sum()
    if duplicates > 0:
