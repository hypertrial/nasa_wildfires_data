...
def verify_meteo_data(
    ...
    for var, (min_val, max_val) in VARIABLE_RANGES.items():
        if var in df.columns:
            if df[var].min() < min_val or df[var].max() > max_val:
                print(
                    f"WARNING: {var} values outside expected range [{min_val}, {max_val}]"
                )
    ...
    if (
        lat_range[0] > min_lat
        or lat_range[1] < max_lat
        or lon_range[0] > min_lon
        or lon_range[1] < max_lon
    ):
        print(
            "WARNING: Meteorological data does not cover the entire region of interest"
        )
        print(f"  - Expected region: [{min_lat}, {max_lat}] × [{min_lon}, {max_lon}]")
        print(
            f"  - Actual coverage: [{lat_range[0]}, {lat_range[1]}] × [{lon_range[0]}, {lon_range[1]}]"
        )
        issues_found = True
    ...
    if data_type == "current":
        ...
        duplicates = df.duplicated(
            subset=["latitude", "longitude", "timestamp", "forecast_time"]
        ).sum()
    ...
    if len(df) > 100:  # Only for larger datasets that are likely gridded
        ...
        if expected_points != len(df):
            print("WARNING: Grid point count inconsistency")
