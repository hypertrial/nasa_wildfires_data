...
def create_grid_cells(min_lat=30, max_lat=70, min_lon=-130, max_lon=-100, grid_size_km=10):
    """
...
def create_grid_edges(lat_bins, lon_bins_by_lat, cell_centers):
    ...
    for lat_idx in range(len(lat_bins) - 1):
        ...
        lon_centers = [(lon_band_bins[i] + lon_band_bins[i+1])/2 for i in range(len(lon_band_bins)-1)]
        ...
        for lon_idx in range(len(lon_centers_by_lat[lat_idx])):
            ...
            if lat_idx < len(lat_bins) - 2:
                ...
                if lat_idx + 1 < len(lon_centers_by_lat):
                    ...
                    closest_lon_idx = np.argmin(np.abs(np.array(next_lon_centers) - current_lon))
                    ...
                    if closest_lon_idx < len(next_lon_centers) - 1:
                        diag_right_id = cell_lookup.get((lat_idx + 1, closest_lon_idx + 1))
                    ...
                    if closest_lon_idx > 0:
                        diag_left_id = cell_lookup.get((lat_idx + 1, closest_lon_idx - 1))
